...
def add_to_dict(loc,pawn):
    if(loc not in killers):
        killers[loc]=[]
    else:
        killers[loc].append(pawn) 
def checkmate_auth(attacker,checked_king):
...
def checkmate_check(loc1,loc2,touched_piece,attacker,checked_king):
    ...
    if(len(final_attackers)==0 and len(king_refuge_locs)==0):
        ...
        return killers
def check_auth(loc1,loc2,touched_piece,attacker,checked_king):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # Set moving as True if you want to move the image without the mouse click
                ...
                if(check=="yes"):
                    auth=check_auth(initial_pos,final_pos,touched_piece_name,attacker,checked_king)
                elif(touched_piece_type=="bishop"):
                    auth=bishop_auth(initial_pos,final_pos)
                elif(touched_piece_type=="pawn"):
                    auth=pawn_move(touched_piece_name,initial_pos,final_pos)
                elif(touched_piece_type=="knight"):
                    auth=knight_auth(initial_pos,final_pos)
                elif(touched_piece_type=="queen"):
                    auth=rook_auth(initial_pos,final_pos)
                    if(auth=="no"):
                        auth=bishop_auth(initial_pos,final_pos)
                elif(touched_piece_type=="rook"):
                    auth=rook_auth(initial_pos,final_pos)
                else:
                    auth=king_auth(initial_pos,final_pos)
                if(auth=="kill"):
                    ...
                    if(check=="no"):
                        for val in regions_under_attack[find_opposite_color(black_king)].values():
                            if(black_king in val):
                                ...
                                check="no"
                    if(check=="yes"):
                        mate=checkmate_auth(attacker,checked_king)
                        if(mate!="checkmate"):
                            print("mate")
                    change_mover()
                elif(auth=="no"):
...
def pawn_movement(touched_pawn,checked_king,loc):
    pawn_list=[]
    if(find_piece_color(touched_pawn)==find_piece_color(checked_king)):
        pawn_loc=piece_locs[touched_pawn]
            if(loc[0]==pawn_loc[0]):
                if(int(loc[1])-int(pawn)==2 and int(pawn_loc[1])==1):
