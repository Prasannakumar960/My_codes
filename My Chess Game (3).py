...
def knight_auth(loc1,loc2):
    ...
    if(loc2 in list2_kng):
        list_kng_attacks=knight_attacks(loc2)
        if(loc2 in piece_locs.values()):
            piece1_color=find_piece_color(find_piece_name(loc1))
            piece2_color=find_piece_color(find_piece_name(loc2))
            if(piece1_color!=piece2_color):
                return ["kill",list_kng_attacks]
            else:
                return ["no"]
        else:
            return ["yes",list_kng_attacks]
    else:
        return ["no"]
        ...
        for event in pygame.event.get():
            ...
            # Set moving as True if you want to move the image without the mouse click
               ...
               # moving_piece.move_ip(event.rel)
                ...
                elif(touched_piece_name[1]=="p"):
                    auth=pawn_move(touched_piece_name,find_piece_loc(cx,cy),find_piece_loc(pk1,pk2))
                elif(touched_piece_name[1:3]=="kn"):
                    auth=knight_auth(find_piece_loc(cx,cy),find_piece_loc(pk1,pk2))
                    if(len(auth)==2):
                        auth,knight_attacks=auth[0],auth[1]
                        print(knight_attacks)
                        regions_under_attack[find_piece_color(touched_piece_name)][touched_piece_name]=knight_attacks
                    else:
                        auth=auth[0]
                elif(touched_piece_name[1]=="q"):
                    auth=rook_auth(find_piece_loc(cx,cy),find_piece_loc(pk1,pk2))
                    if(auth=="no"):
                        auth=bishop_auth(find_piece_loc(cx,cy),find_piece_loc(pk1,pk2))
                else:
                    auth=rook_auth(find_piece_loc(cx,cy),find_piece_loc(pk1,pk2))
                ...
                if(auth=="yes"):
                    ...
                    region_under_attack(touched_piece_name,find_piece_loc(pk1,pk2))
                    change_mover()
                elif(auth=="no"):
...
for piece_name,loc in piece_locs.items():
    ...
    print(piece_name,regions_under_attack)
